...
bool ArdupilotSitlGazeboPlugin::init_ros_side()
{
    ...
    // GPS topic
    topicNameBuf = std::string("/") + _group_label + std::string("/") + _modelName + "/fix";  //----changed
    _gps_subscriber = _rosnode->subscribe(topicNameBuf.c_str(), 1, &ArdupilotSitlGazeboPlugin::gps_callback, this);

    // GPS velocity topic
    topicNameBuf = std::string("/") + _group_label + std::string("/") + _modelName + "/fix_velocity";  //----changed
    _gps_velocity_subscriber = _rosnode->subscribe(topicNameBuf.c_str(), 1, &ArdupilotSitlGazeboPlugin::gps_velocity_callback, this);

    topicNameBuf = std::string("/") + _group_label + std::string("/") + _modelName + "/sonar_down";    //-----additional
    _sonar_down_subscriber   = _rosnode->subscribe(topicNameBuf.c_str(), 1, &ArdupilotSitlGazeboPlugin::sonar_down_callback,   this);
#if SONAR_FRONT == ENABLED
    topicNameBuf = std::string("/") + _group_label + std::string("/") + _modelName + "/sonar_front";    //-----additional
    _sonar_front_subscriber  = _rosnode->subscribe(topicNameBuf.c_str(), 1, &ArdupilotSitlGazeboPlugin::sonar_front_callback,  this);
#endif
    ...
    // _motorSpd_publisher = _rosnode->advertise<mav_msgs::CommandMotorSpeed>(topicNameBuf.c_str(), 10); ---delete
    _motorSpd_publisher = _rosnode->advertise<mav_msgs::Actuators>(topicNameBuf.c_str(), 10); // ----additional
...
{
    //boost::shared_ptr<mav_msgs::CommandMotorSpeed> cmdMotSpd_msg = boost::make_shared<mav_msgs::CommandMotorSpeed>(); -----delete
    boost::shared_ptr<mav_msgs::Actuators> cmdMotSpd_msg = boost::make_shared<mav_msgs::Actuators>();  //-----additional
    ...
    //cmdMotSpd_msg->motor_speed.clear(); ---delete
    cmdMotSpd_msg->angles.clear(); //---additional
    cmdMotSpd_msg->angular_velocities.clear(); //---additional
    cmdMotSpd_msg->normalized.clear(); // ---additional
    for (i=0; i<_nbMotorSpeed; i++)
       //cmdMotSpd_msg->motor_speed.push_back(_cmd_motor_speed[i]); ---delete
      { cmdMotSpd_msg->angular_velocities.push_back(_cmd_motor_speed[i]); //---additional
        cmdMotSpd_msg->angles.push_back(0); // ----additional
        cmdMotSpd_msg->normalized.push_back(0); } // ---additional
