...
// ros::Publisher publisher = nh.advertise<geometry_msgs::TwistStamped>("/uav/mavros/setpoint_velocity/cmd_vel", 1000);
    ...
    ros::Publisher  local_pos_pub  = nh.advertise<geometry_msgs::PoseStamped>("/" + group_name + "/mavros/setpoint_position/local",10); //--changed
    ros::Subscriber local_pose_sub = nh.subscribe("/" + group_name + "/mavros/local_position/pose", 10, &Loop::set_yaw, &loop);   // ---changed
   ...
   int counter(0);
    ...
    while (ros::ok())
    {
        ...
        if (counter>5)
        {
            ...
            double target_rotation=60*3.14159/180;
            ...
            poseMsg.header.frame_id="world"; // ---additional
